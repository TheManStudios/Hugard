...
# If any variables are missing, prompts the user to set them
if TOKEN == "" or OWNER_ID == "" or COINBASE_API == "":
    try:
        print("Oh no! It looks like you’ve forgotten to fill out your \"secrets/.env\" file. But don't worry! I'll help you set it up in no time! (Press Ctrl+C if you don't want my help)")
        if TOKEN == "":
            TOKEN = input("I see that your Discord bot's token is missing. Please give it to me: ")
        if OWNER_ID == "":
            OWNER_ID = input("Sadly your Discord ID is missing. Please give it to me: ")
        if COINBASE_API == "":
            COINBASE_API = input("It looks like your Coinbase API key is missing. Please give it to me: ")
        with open("secrets/.env", "w") as file:
            file.write(f"TOKEN={TOKEN}\nOWNER_ID={OWNER_ID}\nCOINBASE_API={COINBASE_API}")
        exit()
    except KeyboardInterrupt:
        print("\n\nYou don't what my help? Okay then, bye!")
        with open("secrets/.env", "w") as file:
            file.write(f"TOKEN={TOKEN}\nOWNER_ID={OWNER_ID}\nCOINBASE_API={COINBASE_API}")
        exit()
...
async def sanitize_text(text):
    return re.sub(r'\s+', ' ', ''.join(char for char in html.escape(str(text)) if ord(char) >= 32)).translate(str.maketrans('', '', '<>{}\'"\\$|;*?\\&')).strip()

# Handles events when the bot is ready
@bot.event
async def on_ready():
    if len(bot.guilds) > 1:
        print("Oh no! I'm in more than 1 server. Please remove me from all extra servers and try again.")
        exit()
    await init_db()
    ...
    @bot.slash_command(guild_ids=[bot.guilds[0].id],description="Ping me to see how fast I respond!")
    async def ping(ctx):
        await ctx.respond(f"Pong! {bot.latency * 1_000:.0f}ms", ephemeral=True)

    #Provides information about the bot
    @bot.slash_command(guild_ids=[bot.guilds[0].id], description="Find out a bit more about me!")
    async def about(ctx):
    ...
    @bot.slash_command(guild_ids=[bot.guilds[0].id], description="See payment metrics for a specific user or the entire server!")
    ...
    @bot.slash_command(guild_ids=[bot.guilds[0].id], description="Check your own payment history in the server!")
    ...
    @bot.slash_command(guild_ids=[bot.guilds[0].id], description="Send your owner some money!")
